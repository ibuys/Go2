...
//  Go2 Spotlight Importer
//
//  Created by Jonathan Buys on 10/27/10.
//  Copyright (c) 2010 Farmdog Software. All rights reserved.
//
			   ...
			   CFStringRef pathToFile)
{
    ...
    if ([(NSString *)contentTypeUTI isEqualToString:@"farmdog.go2.xml"]) {
        ...
        if ( error == NULL ) {
			...
			NSObject *contentToIndex = [metadata objectForKey: @"hostName"];
    ...
    } else if ([(NSString *)contentTypeUTI isEqualToString:@"farmdog.go2.xml"]) {
